...
BOOST_FIXTURE_TEST_CASE( test_coma_model_density_correction_parameters, TestDataPaths )
{
    ...
    Eigen::MatrixXd accelerationPartial = comaModel.getDensityCorrectionAccelerationPartial( time, currentAcceleration );
    ...
    accelerationPartial = comaModel.getDensityCorrectionAccelerationPartial( time, currentAcceleration );
    BOOST_CHECK_EQUAL( accelerationPartial.rows( ), 3 );
    ...
    {
        BOOST_CHECK_SMALL( ( accelerationPartial.col( parameterIndex ) - currentAcceleration * expectedMultipliers( parameterIndex ) ).norm( ),
                           1.0e-15 );
    }
    ...
    BOOST_CHECK_THROW( comaModel.setArcWiseDensityCorrectionArcStartTimes( std::vector< double >( { secondArcStartTime, startTime } ) ),
                       std::runtime_error );
    ...
    BOOST_CHECK_CLOSE( comaModel.getDensity( radius, longitude, latitude, firstArcTime ), 2.0 * firstArcNominalDensity, 1.0e-10 );
    ...
    BOOST_CHECK_CLOSE( comaModel.getDensity( radius, longitude, latitude, secondArcTime ), 0.25 * secondArcNominalDensity, 1.0e-10 );
    arcWiseAccelerationPartial = comaModel.getArcWiseDensityCorrectionAccelerationPartial( secondArcTime, currentAcceleration );
    for( int parameterIndex = 0; parameterIndex < expectedMultipliers.rows( ); ++parameterIndex )
    {
        BOOST_CHECK_SMALL( arcWiseAccelerationPartial.col( parameterIndex ).norm( ), 1.0e-15 );
        BOOST_CHECK_SMALL(
                ( arcWiseAccelerationPartial.col( 7 + parameterIndex ) - currentAcceleration * expectedMultipliers( parameterIndex ) )
                        .norm( ),
                1.0e-15 );
    }
    ...
    BOOST_CHECK_CLOSE( comaModel.getDensity( radius, longitude, latitude, firstArcTime ), firstArcNominalDensity, 1.0e-10 );
}
